...
begin
  ...
  {The exception raised in output_message should disallow these lines to execute}
...
begin
  ...
  { Probably the next line will get an access violation, but I do not think so}  
  { Let the memory manager delete any temp files before we die }
  jpeg_destroy(cinfo);

  { raise an exception }
  raise EJPEGException.Create(buffer);
end;
