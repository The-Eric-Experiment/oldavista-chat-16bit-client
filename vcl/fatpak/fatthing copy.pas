...
type
  ...
  protected
    ...
    procedure SetAsIRCText(Value: String);
  ...
  public
    ...
    property AsIRCText: String read GetAsText write SetAsIRCText;
  ...
  protected
    ...
    procedure SetSelStart(ALineIdx, APartIdx, ACharIdx: Integer);
    procedure SetSelEnd(ALineIdx, APartIdx, ACharIdx: Integer);
  ...
  public
    ...
    procedure CopySelection(SelStart, SelEnd: TSelPos);
    ...
    function InSelection(const LineIdx, PartIdx, CharIdx: Integer): Boolean;
...
const
  AuthorEmail = 'gasper.kozak@email.si';
  MAX_LINES   = 9999;
  CalText     = 'W|j_`';
...
begin
  ...
  if P.x <= DrawDim.Right then
    Result := P.y >= DrawDim.Top + LineHeight else
  ...
  case FType of
    ...
    begin
      ...
      if Size > 0 then
        for I := 1 to Size do
        begin
          Selected := Memo.InSelection(FLine.Index, Index, I);
          if Selected then
            Break;
        end;
    ...
    begin
      ...
      begin
        ...
        begin
          DrawRect := Rect(Left, Top, Left + FWidth, Top + LineHeight);
...
procedure TFatLine.SetAsIRCText(Value: String);
var Tag, Src, ALink: String;
  NewPart: TFatPart;
  PrevFontColor, PrevBackColor,
  PFontColor, PBackColor: TColor;
  PrevStyle, PStyle: TFontStyles;
  TxColor, BkColor, EndLinkPos: Integer;
begin
  Updating := True;
  ...
  ALink := '';
  PrevFontColor := FLines.FMemo.Font.Color;
  PrevBackColor := FLines.FMemo.Color;
  PrevStyle := FLines.FMemo.Font.Style;

  PFontColor := PrevFontColor;
  PBackColor := PrevBackColor;
  PStyle := PrevStyle;
  ...
  repeat
    ...
    if Tag = TAG_BOLD then
    begin
      InvertFontStyle(PStyle, fsBold);
      NewPart.Style := PStyle;
    end else
    if Tag = TAG_UNDERLINE then
    begin
      InvertFontStyle(PStyle, fsUnderline);
      NewPart.Style := PStyle;
    end else
    if Copy(Tag, 1, 1) = TAG_COLOR then
    begin
      if Length(Tag) = 1 then
      begin
        NewPart.FontColor := PrevFontColor;
        NewPart.BackColor := PrevBackColor;
      end else
      begin
        StripColorTag(Tag, TxColor, BkColor);
        if TxColor >= 0 then
        begin
          NewPart.FontColor := MircColors[TxColor];
          if BkColor >= 0 then
            NewPart.BackColor := MircColors[BkColor] else
            NewPart.BackColor := PBackColor;
        end;
      end;

      PFontColor := NewPart.FontColor;
      PBackColor := NewPart.BackColor;
    end else
    if Tag = TAG_INVERSE then
    begin
      NewPart.FontColor := PBackColor;
      NewPart.BackColor := PFontColor;
      PBackColor := NewPart.BackColor;
      PFontColor := NewPart.FontColor;
    end else
    if Tag = TAG_NORMAL then
    begin
      NewPart.FontColor := PrevFontColor;
      NewPart.BackColor := PrevBackColor;

      NewPart := Add;
      NewPart.Style := PrevStyle;

      PStyle := PrevStyle;
      PFontColor := PrevFontColor;
      PBackColor := PrevBackColor;
    end else
    if Tag = TAG_BEGINLINK then
    begin
      EndLinkPos := Pos(TAG_ENDLINK, Src);
      if EndLinkPos = 0 then
        EndLinkPos := Length(Src) + 1;

      ALink := RemoveTags(Copy(Src, 1, EndLinkPos));
    end else
    if Tag = TAG_ENDLINK then
      ALink := '' else
      NewPart.Delete;
  until Tag = '';

  FLines.FMemo.Font.Style := PrevStyle;
  Updating := False;
end;
...
begin
  ...
  InflateRect(R, -3, -4);
  ALeft := R.Left + FBarHoriz.Position;
  ATop := R.Top;

  All := Lines.Count;
  if (All = 0) or (TopIndex >= Lines.Count) then
  begin
    FBarVert.Max := 0;
    FBarHoriz.Max := 0;
    Exit;
  ...
  AHeight := FLines.HeightLines - (R.Bottom - R.Top) div LineHeight;
  if AHeight >= 0 then
    FBarVert.Max := AHeight
  else
    FBarVert.Max := 0;
  ...
  if not (dfWordWrap in DrawFlags) then
  begin
    for I := 0 to Lines.Count - 1 do
      if Lines[I].Width > AWidth then
        AWidth := Lines[I].Width;

    FBarHoriz.Max := AWidth;
  end;
end;

procedure TFatMemo.Paint;
begin
  TCustomControl.Paint(Self);  { Call the inherited Paint method explicitly }
...
begin
  ...
  if Assigned(FOnScrollHoriz) then
    FOnScrollHoriz(Self);
end;
...
begin
  ...
  if (mbLeft = Button) and (Shift = [ssLeft]) and (OverLine <> NIL) and (OverPart <> NIL) then
  begin
    FClick := True;
    ChIdx := OverPart.PointOverChar(X, Y, R);
    SetSelStart(OverLine.Index, OverPart.Index, ChIdx);
    with FSelStart.Pos do
    begin
      x := R.Left;
      y := R.Top;
    end;
...
begin
  if (mbLeft = Button) and FSelecting then
  begin
  ...
  if (mbLeft = Button) and FClick then
    FClick := True{*if Assigned(OnClick) then
        OnClick(Self);*};
...
begin
  ...
  if (Shift = [ssLeft]) and (OverLine <> NIL) and (OverPart <> NIL) then
  begin
    FSelecting := True;
    FClick := False;

    ChIdx := OverPart.PointOverChar(X, Y, R);
    SetSelEnd(OverLine.Index, OverPart.Index, ChIdx);
    with FSelEnd.Pos do
    begin
      x := R.Right;
      y := R.Bottom;
...
begin
  ...
  {*if (FOverPart <> NIL) and FOverPart.IsLink then
    begin
      ...
      if Link = 'mailto:' + AuthorEmail then
...
procedure TFatMemo.SetSelEnd(ALineIdx, APartIdx, ACharIdx: Integer);
begin
  with FSelEnd do
  begin
    LineIdx := ALineIdx;
...
function TFatMemo.InSelection(const LineIdx, PartIdx, CharIdx: Integer): Boolean;
var SStart, SEnd, Sel: LongInt;
begin
  Result := False;
  if not Selecting then
    Exit;

  SStart := SelStart.LineIdx * 10000000 + SelStart.PartIdx * 1000 + SelStart.CharIdx;
  SEnd := SelEnd.LineIdx * 10000000 + SelEnd.PartIdx * 1000 + SelEnd.CharIdx;
  Sel := LineIdx * 10000000 + PartIdx * 1000 + CharIdx;

  Result := (SStart <= Sel) and (Sel <= SEnd);

  {if (LineIdx < SelStart.LineIdx) or (LineIdx > SelEnd.LineIdx) then
    Exit;

  if ((LineIdx = SelStart.LineIdx) and (PartIdx < SelStart.PartIdx)) or
  if ((LineIdx = SelEnd.LineIdx) and (PartIdx > SelEnd.PartIdx)) then
    Exit;}
end;

procedure TFatMemo.CopyToClipboard;
var SelStart, SelEnd: TSelPos;
begin
  ...
  begin
    ...
    PartIdx := Lines[LineIdx].Count - 1;
    if PartIdx >= 0 then
      CharIdx := Length(Lines[LineIdx].Parts[PartIdx].Text);
  end;

  CopySelection(SelStart, SelEnd);  
end;

procedure TFatMemo.CopySelection(SelStart, SelEnd: TSelPos);
var S: String;
  CharLo, CharHi,
  Len, LineCnt, PartCnt, PartLo, PartHi: Integer;
  Empty: Boolean;
  Buf: PChar;
  P: TFatPart;
begin
  if (SelStart.LineIdx > SelEnd.LineIdx) or
    ((SelStart.LineIdx = SelEnd.LineIdx) and (SelStart.PartIdx > SelEnd.PartIdx)) then
    Exit;

  S := '';
  for LineCnt := SelStart.LineIdx to SelEnd.LineIdx do
  begin
    PartLo := 0;
    PartHi := Lines[LineCnt].Count - 1;
    Empty := PartHi = - 1;

    if LineCnt = SelStart.LineIdx then
      PartLo := SelStart.PartIdx;

    if LineCnt = SelEnd.LineIdx then
      PartHi := SelEnd.PartIdx;

    if (LineCnt > SelStart.LineIdx) and (LineCnt < SelEnd.LineIdx) then
    begin
      PartLo := 0;
      PartHi := Lines[LineCnt].Count - 1;
    end;
    ...
    if not Empty then
      for PartCnt := PartLo to PartHi do
      begin
        P := Lines[LineCnt].Parts[PartCnt];

        CharLo := 1;
        CharHi := Length(P.Text);
        if CharHi = 0 then
          Continue;

        if (LineCnt = SelEnd.LineIdx) and (PartCnt = SelEnd.PartIdx) then
          CharHi := SelEnd.CharIdx;

        if (LineCnt = SelStart.LineIdx) and (PartCnt = SelStart.PartIdx) then
        begin
          CharLo := SelStart.CharIdx;
          CharHi := CharHi - CharLo + 1;
        end;

        S := S + Copy(P.Text, CharLo, CharHi);
      end;
