...
Legal issues: Copyright (C) 1996-2005 by Fran�ois PIETTE
...
{$IFDEF USE_SSL}
    IcsSSLEAY, IcsLIBEAY, IcsSslConnector, ExtCtrls,
{$ENDIF}
  WSocket,
...
const
  FtpCliVersion      = 299;
  CopyRight : String = ' TFtpCli (c) 1996-2005 F. Piette V2.99 ';
...
{$IFDEF USE_SSL}
    if (FFctPrv in [ftpFctAuthSsl]) and
...
begin
{$IFDEF DEBUG_OUTPUT}
OutputDebugString('TCustomFtpCli.DataSocketGetSessionConnected');
{$ENDIF}
...
{   FDataSocket.OnDisplay        := FOnDisplay; } { Debugging only }
    FDataSocket.HSocket          := aSocket;
{$IFDEF USE_SSL}
    if TCustomSslWSocket(FDataSocket).SslEnable then
        TCustomSslWSocket(FDataSocket).AcceptSslHandshake;
{$ENDIF}
    FDataSocket.ComponentOptions := [wsoNoReceiveLoop];   { 26/10/02 }
...
{$IFDEF NEVER_USE_SSL}
        { Test }
        if PeekMessage(Msg, FDataSocket.Handle, WM_ASYNCSELECT, WM_ASYNCSELECT, PM_NOREMOVE) then
            WinProcs.OutputDebugString('PeekMessage 2')
        else
...
{$IFDEF DEBUG_OUTPUT_MSGS}
        OutputDebugString('DataSocketPutDataSent ' + IntToStr(Count));
{$ENDIF}
...
__DataSocket := FDataSocket;
{$IFDEF DEBUG_OUTPUT}
    OutputDebugString('FDataSocket.Connect');
{$ENDIF}
{$ENDIF}
        try
            FDataSocket.Connect;
...
{$IFDEF USE_SSL}
        ...
        if TSslFtpClient(Self).FProtLevelSent = 'P' then begin
            FLastResponse := '550 SSL on data channel require passive mode';
            FStatusCode   := 550;
            SetErrorMessage;
            FDataSocket.Close;
            FRequestResult := FStatusCode;
