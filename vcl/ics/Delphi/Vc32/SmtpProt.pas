...
begin
    if (FItemCount >= 0) and (FRequestResult <> 0) then  {A.G. 1/3/05 start}
    begin
...
begin
    ...
    if not FOwnHeaders then begin
        ...
        if FConfirmReceipt and (Length(Trim(FHdrFrom)) > 0) then        {AG}
...
begin
    ...
    if FItemCount < FHdrLines.Count then begin
        ...
        {A.G.}
        { Calling Abort in TSmtpCli.TriggerGetData return here,
...
function GenerateMessageID : String;                                     {AG}
begin
    Result := FormatDateTime('yyyymmddhhnnsszzz', Now + TimeZoneBias) + '.' +
