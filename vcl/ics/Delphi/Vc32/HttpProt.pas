...
const
    HttpCliVersion       = 188;
    CopyRight : String   = ' THttpCli (c) 1997-2005 F. Piette V1.88 ';
    DefaultProxyPort     = '80';
{$IFDEF DELPHI1}
...
begin
    ...
    { to authenticate                                                    }
    Result := FALSE;
    if httpoNoBasicAuth in FOptions then
        Exit;

    { if you place this code in GetHeaderLineNext, not each time will be }
    { called ...                                                         }
    if (FAuthBasicState = basicMsg1) and (FStatusCode <> 401) then
...
{$IFDEF UseNTLMAuthentication}
                    if not (((FNTLMAuthType = ntlmAuthNormal) and (FAuthNTLMState = ntlmMsg1)) or
                       ((FNTLMAuthType = ntlmAuthProxy) and (FProxyAuthNTLMState = ntlmMsg1))) then begin
                    TriggerSendBegin;
                    FAllowedToSend := TRUE;
...
{$IFDEF DEBUG_OUTPUT}
                        OutputDebugString('ChunkLength = ' + IntToStr(FChunkLength));
{$ENDIF}
                        break;
                    end;
...
{$IFDEF DEBUG_OUTPUT}
            OutputDebugString('httpChunkDone');
...
begin
    if FAuthNTLMState = ntlmNone then begin
        FNTLMAuthType := ntlmAuthNormal;
{$IFDEF DEBUG_OUTPUT}
        OutputDebugString('starting NTLM authentication');
{$ENDIF}
...
begin
    if FProxyAuthNTLMState = ntlmNone then begin
        FNTLMAuthType := ntlmAuthProxy;
{$IFDEF DEBUG_OUTPUT}
        OutputDebugString('starting Proxy NTLM authentication');
{$ENDIF}
        CleanupRcvdStream; { What we are received must be removed }
        CleanupSendStream;
        FReceiveLen         := 0;
...
begin
    if FAuthBasicState = basicNone then begin
        FBasicAuthType := basicAuthNormal;
{$IFDEF DEBUG_OUTPUT}
        OutputDebugString('starting Basic authentication');
{$ENDIF}
        CleanupRcvdStream; { What we are received must be removed }
        CleanupSendStream;
        FReceiveLen       := 0;
...
begin
    if FProxyAuthBasicState = basicNone then begin
        FBasicAuthType := basicAuthProxy;
{$IFDEF DEBUG_OUTPUT}
        OutputDebugString('starting Proxy Basic authentication');
{$ENDIF}
        CleanupRcvdStream; { What we are received must be removed }
        CleanupSendStream;
        FReceiveLen          := 0;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF USE_SSL}
procedure THttpCli.SslHandshakeDone(Sender : TObject; ErrCode : Word);
begin
    try
        FNext := GetHeaderLineNext;
        StateChange(httpWaitingHeader);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function THttpCli.GetNTLMMessage3: String;
var
    Hostname : String;
begin
    { get local hostname }
    ...
    if FNTLMAuthType = ntlmAuthProxy  then begin
