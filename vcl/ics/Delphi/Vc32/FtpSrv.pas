    ...
    try
        ...
        if Allowed then begin
            ...
            if (not (ftpCwdCheck in Client.Options)) or DirExists(Client.Directory) or
                  (Length(Client.Directory) <= 3) or  { angus V1.39 }
...
{$ELSE}
            Windows.GetTempFileName (PChar(Client.Directory), 'FTP', 0, @buffer[0]);
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TFTPServer.FtpSrvAllocateHWnd(Method: TWndMethod) : HWND;
begin
{$IFDEF NOFORMS}
    Result := XSocketAllocateHWnd(Self);
    SetWindowLong(Result, GWL_WNDPROC, LongInt(@FtpSrvWindowProc));
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFTPServer.FtpSrvDeallocateHWnd(WHandle : Cardinal);
begin
{$IFDEF NOFORMS}
